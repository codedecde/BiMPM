...
from allennlp.nn.util import get_lengths_from_binary_sequence_mask, \
    masked_max, masked_mean, masked_softmax
...
class BiMpmMatching(nn.Module):
    ...
    def from_params(cls, params: Params):
        hidden_dim = params.pop("hidden_dim", 100)
        num_perspectives = params.pop("num_perspectives", 20)
        share_weights_between_directions = params.pop("share_weights_between_directions", True)
        is_forward = params.pop("is_forward", None)
        with_full_match = params.pop("with_full_match", True)
        with_maxpool_match = params.pop("with_maxpool_match", True)
        with_attentive_match = params.pop("with_attentive_match", True)
        with_max_attentive_match = params.pop("with_max_attentive_match", True)
        return cls(
            hidden_dim=hidden_dim,
            num_perspectives=num_perspectives,
            share_weights_between_directions=share_weights_between_directions,
